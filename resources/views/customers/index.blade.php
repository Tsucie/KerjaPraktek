          ...
          <form class="form-horizontal" action="" method="" id="form">
              <div class="box-body">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-12">
                        <input type="text" class="form-control form-inputs" id="total-ord" disabled>
                    </div>
                    <div class="col-sm-12">
                        <input type="text" class="form-control form-inputs" id="vnu-ord" disabled>
                    </div>
                    <div class="col-sm-12">
                        <input type="text" class="form-control form-inputs" id="pdct-ord" disabled>
                    </div>
                  </div>
              </div>
          </form>
...
<div class="modal fade" id="DeleteModal" tabindex="-1" role="dialog" aria-labelledby="DeleteModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="DeleteModalTitle">Hapus Data Customer</h5>
                    ...
                    <tbody class="list">
                        @forelse ($data as $item)
                            <tr>
                                ...
                                <td class="action">
                                    <a class="btn btn-sm btn-info btn-table" data-toggle="tooltip" data-html="true" title="Lihat Detail Data" id="btndetail{{ $loop->index }}" role="button" data_id="{{ $item->cst_id }}" data_vnu_ord="{{ $item->vnu_ord }}" data_pdct_ord="{{ $item->pdct_ord }}" onclick="ShowDetails(this)">Detail</a>
                                    @if ($item->vnu_ord == 0 && $item->pdct_ord == 0)
                                        <button class="btn btn-sm btn-danger btn-table" data-toggle="tooltip" data-html="true" title="Hapus Data Customer" id="btnhapus{{ $loop->index }}" role="button" data_id="{{ $item->cst_id }}" data_name="{{ $item->cst_name }}" onclick="DeleteUser(this)">Hapus</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
